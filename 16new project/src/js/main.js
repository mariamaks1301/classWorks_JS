...
let filterSelectorPruce = document.querySelector('.main__assort-select-price');
let filterSelectorCAtegory = document.querySelector('.main-assort-select-category');
let filterSelectorGender = document.querySelector('.main-assort-select-gender');
let filetrSelectorBrand = document.querySelector('.main-assor-selector-brand');
...
let filterGender = '';
let filterBrand = '';
let filterSeachValue = '';

const getShoes = ()=>{
    shoesList.innerHTML = '';
    fetch(url + `${filterPrice.length ? '_sort=price&_order=' + filterPrice + '&' : ''}${filterCategory.length ? 'category='+ filterCategory + '&' : ''}${filterGender.length ? 'gender=' + filterGender + '&' : ''}${filterBrand.length ? 'brand=' + filterBrand + '&' : ''}${filterSeachValue.length ? 'title_like=' + filterSeachValue + '&' : ''}`)
            ...
            .then((resolve)=>{
                resolve.forEach(item => {
                    ...
                    <div class="main__assort-mainContent-shoes">
                        ...
                        <p class="main__assort-mainContent-product-gender">${item.gender === 'man' ? 'Mужские':'Женские'}</p>
                        <p class="main__assort-mainContent-product-category">${item.category === 'street' ? 'Уличные': item.category === 'run' ? 'Беговые': item.category === 'train' ? 'Тренировочные' : 'Гимнастические'}</p>
...
filetrSelectorBrand.addEventListener('change', (e)=>{
    filterBrand = e.target.value
    getShoes()
});
getShoes();

filterSeachValue.addEventListener('keyup', (e)=>{
    filterSeachValue = e.target.value
    getShoes()
});
getShoes();

basketBtn.addEventListener('click', (e)=>{
    basketBtn.classList.toggle('show');
})